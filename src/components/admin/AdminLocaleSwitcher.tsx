...
export function AdminLocaleSwitcher() {
  ...
  function navigate(target: LocaleCode) {
    ...
    // Strip any existing /{shippedLocale} prefix from the current pathname
    const localePrefix = new RegExp('^/(' + SHIPPED_LOCALES.join('|') + ')(?=/|$)')
    const barePath = pathname.replace(localePrefix, '') || '/'

    // Admin: English (DEFAULT_LOCALE) is served unprefixed at /admin/...
    // All other locales get a /{locale} prefix: /fr/admin/..., /ur/admin/...
