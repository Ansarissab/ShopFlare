...
import type { SizeOption, SizePickerProps } from '@/lib/types/store'
          ...
          return (
            <div key={size.id} className="relative flex flex-col items-center gap-1">
              <button
                ...
                aria-disabled={isOOS}
              >
                <span className={cn('font-medium', isOOS && 'line-through')}>
                  {size.size}
                </span>
              ...
              {isLow && (
                <Badge variant="destructive" className="text-xs px-1.5 py-0 h-4 pointer-events-none">
